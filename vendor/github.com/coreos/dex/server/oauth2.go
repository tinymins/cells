...
func (s *Server) newAccessToken(connID string, claims storage.Claims) (string, error) {
	...
	if err != nil {
		...
		return "", fmt.Errorf("failed to marshal offline session ID: %v", err)
	}
	...
	if err != nil {
		s.logger.Errorf("failed to serialise access token: %v", err)
		return "", err
	}

	accessToken, err := s.Encrypt(jsonStr)
	if err != nil {
...
func (s *Server) Encrypt(message []byte) (string, error) {
	keys, err := s.storage.GetKeys()
	if err != nil {
...
func (s *Server) Decrypt(message string) ([]byte, error) {
	keys, err := s.storage.GetKeys()
	if err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("unable to parse encrypted message")
	}
